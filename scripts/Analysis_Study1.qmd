...
An increase in CNI complexity was observed with an increase in the number of items (@fig-plot-model-complexity). 
...
#Range of CNI values for each model
cni_plot_details %>% 
  pull(cni_mod) %>% 
  map_dfr(~{
    ...
    plot_obj$mod_slopes %>%
      tibble() %>% 
      reframe(range = range(estimate)) %>%
      mutate(model = .x,
             range_value = c("low", "high"))
  }) %>%
  # Reorder columns to have 'model' first
  select(model, everything()) %>% 
  pivot_wider(names_from = "range_value", values_from = "range") 
```
...
export(CNImods_table1, "CNImods_table1.xlsx")
```
...
cni.ContMod_tables$mindsetCNI.pty7 %>% 
  View()
```
...
hetero_plot("mindsetCNI_mod")
names(all_swb.cni_mods_heterosk)
