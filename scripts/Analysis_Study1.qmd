...
```{r tbl-heterogeneity, results='asis', message=F}
...
```{r}
hetero_plot = function(model, title){ 
  plot = model %>%
augment %>%
sample_n(1000) %>%
...
height = .1, width = 0) + labs(x = "Est CNI",
       y = "Residual", title = title) + theme_bw()
return(plot) }

names(all_swb.cni_mods_heterosk)

hetero_wb2 = hetero_plot(all_swb.cni_mods_heterosk$mindsetCNI_mod$linear_lm, "Well-being\n(quadratic)")
```
