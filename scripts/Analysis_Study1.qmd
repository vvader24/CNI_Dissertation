...
pacman::p_load(tidyverse, dplyr, rio, expss, scales, glue,
              ...
              lavaan,semTools,#Measurement Invariance
               ...
               patchwork,gridExtra, ggplot2, grid, ggtext, tools, ggcorrplot, #plots
...
```{r tbl-DescriptiveTable, echo=FALSE, results='asis'}
...
# Function to calculate average number of observations per participant
#avg.obs_per_participant <- \(model) {
  s <- summary(model)
  tibble(
    total_obs = s$devcomp$dims["N"], # Total number of observations
    total_participants = s$ngrps["pID:country"] # Total number of participants (pID:country)
  ) %>%
    mutate(avg_obs_per_participant = total_obs / total_participants) %>%
    select(avg_obs_per_participant) %>% 
    pull(avg_obs_per_participant)
}
...
all_mods_ctry.trends %>% 
  ...
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high), position = position_dodge(width = 0.5), width = 0.4) +
...
specify_var_names <- \(data){
  data %>% 
    ...
    modtr = case_when(
      ...
      str_detect(modtr, "fntcYug") ~ "Balkan-based Extrm",
      str_detect(modtr, "fntcIslam") ~ "Islam-based Extrm",
...
cni.contMod_tbl_details %>% 
  select(-caption) %>% 
  pwalk(function(table_name, caption2, ...) {
    cat("\n\n")
    print(
      pluck(cni.ContMod_tables, table_name) %>% 
  ...
  combined_plot <- ggplot(all_data, 
    ...
    labs(
      ...
      linetype = "Association") +
            theme_bw() +
    theme(text = element_text(family = "serif", size = 13.2), legend.position = "bottom")+
...
#| cache: true
cni_types <- unique(sub("CNI.*", "CNI", names(cni.ContMod_tables)))
...
strongest5_contMods_CNI <- map_dfr(cni_types, ~strongest5_contMods(.x, cni.ContMod_tables)) %>%
  specify_var_names() %>% 
  with_groups(modtr, ~ mutate(.x, avg_abs_estimate = mean(abs(estimate)))) %>% 
  ...
  mutate(label_color = case_when(
    str_detect(modtr_type, "\\.pty7$") ~ "#c76e00",
    str_detect(modtr_type, "\\.pty6$") ~ "#c76e00",
    str_detect(modtr_type, "\\.mindset$") ~ "#008000",
    TRUE ~ "black"))

# this approach ensures that the order of coloring is right
...
strongest5_contMods_CNI %>% 
 ggplot(aes(x = modtr, y = estimate, color = cni_type, group = interaction(modtr))) +
   geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width = 0, alpha = 0.3, linewidth = 3.8, position = position_dodge(width = 0.4), linetype = "solid")+
   geom_point() + 
  scale_color_manual(values = c("mindsetCNI" = "#0074B7", "ismCNI"  = "#6082B6", "fantcCNI"  = "#70d6ff", "globeCNI"  = "#a0c4ff", "pty6CNI" = "#ff7477",  "pty7CNI" = "#ff70a6")) +
   labs(x = NULL, 
        y = "CNI estimate",
        color = "CNI Types",  
   ...
   theme(plot.title.position = "plot", 
      ...
      size = 13),
      plot.caption = element_markdown(hjust =1.4, size = 8.3) )
```
...
final_table <- map_dfr(cni_types, ~strongest5_contMods(.x, cni.ContMod_tables)) %>%
  specify_var_names() %>% 
  with_groups(modtr, ~ mutate(.x, avg_abs_estimate = mean(abs(estimate)))) %>% 
  mutate(modtr = fct_reorder(modtr, avg_abs_estimate, .desc = FALSE)) %>% 
...
```{r Agreeableness, eval=FALSE, echo=FALSE, include=FALSE}
# Agreeableness = scP_Agree.comm
# Assertiveness = scM_globe_asrt.comm

#mindsetCNI.agr
mindsetCNI.assrt <- data1 %>% 
  select(pID, matches("scM_globe_asrt.comm")) %>% 
  #mean centering the moderator score cols
  #since this standardization is col-wise, running pty7 is not an issue
   mutate_if(is.numeric, 
             ~. - mean(., na.rm = T)) %>% 
  pivot_longer(-pID, 
                names_to = "modtr", #moderator name
                values_to = "modtr_score") %>% # moderator score
  full_join(mindsetCNI_profiles$self.ctry_profiles, by = c("pID"), relationship = "many-to-many") %>% 
  lmer(z_response ~ z_ctry_response * modtr_score + z_ctry_response * I(modtr_score^2) + (-1 + z_ctry_response | pID), data = .)

  plot_model(mindsetCNI.assrt, type = "pred", terms = c("z_ctry_response[all]", "modtr_score [meansd]")) +
theme_bw() #+
    # labs(x = "CNI", y = "SWB")+
    # theme(plot.title = element_blank())
...
```{r table-cni.swb_effects, echo=FALSE}
#| cache: true
cni.swbMod_tables %>%
...
```{r MeasurementInvariance_Table, echo=FALSE}
...
White's Test for Heteroscedasticity: This test assesses whether the variance of the residuals is constant across all levels of the independent variable(s). A significant W statistic indicates the presence of heteroscedasticity.
...
pty5CNI_mod <- mindsetCNI_profiles$self.ctry_profiles %>% 
  full_join(mindsetCNI_profiles$overallM_profiles, by = "item") %>% add_CNIformula(.)
...
#| tbl-cap: "Heteroscedasticity of SWB across CNI types"
#| cache: true
...
ggplot(aes(x = CNI, y = .resid)) + 
    geom_jitter(alpha = .3,
height = .1, width = 0) + labs(x = "Estimated CNI",
       y = "Residual") +
     # xlim(-0.8, 1) +
     # ylim(-8, 10) + 
        theme_bw()}
...
```{r swb.cni-tbl, results='asis', echo=FALSE, eval=TRUE}
#| tbl-cap: "Prediciting SWB from CNI"
#| cache: true
swb.cni_results <- map_depth(all_swb.cni_mods, .depth = 2, tidy) %>% 
map(., bind_rows, .id = "model") %>%
  map_dfr(bind_rows, .id = "cni_model") %>%
  filter(term %in% c("CNI", "I(CNI^2)"), effect == "fixed") %>%
  mutate(
    term = ifelse(term == 'I(CNI^2)', '$b^2$', 'b'),
    across(c("estimate", "df"), printnum),
    p.value = printp(p.value, add_equals = TRUE),
    estimate = paste0(estimate, "(", df, ") p ", p.value),
    level = case_when(
      str_ends(model, "indv") ~ "individual",
      str_ends(model, "comm") ~ "country",
      TRUE ~ NA_character_),
    model_type = str_remove(model, "\\.indv|\\.comm$")) %>%
  select(cni_model, level, model_type, term, estimate) %>%
  pivot_wider(
    names_from = c(model_type),
    values_from = estimate) %>% 
   arrange(desc(level)) %>% 
 mutate(cni_model = case_when(
    cni_model == "mindsetCNI_mod" ~ "Mindset CNI",
    cni_model == "ismCNI_mod" ~ "Isms CNI",
    cni_model == "globeCNI_mod" ~ "Globe CNI",
    cni_model == "fantcCNI_mod" ~ "Fanaticism CNI",
    cni_model == "pty6CNI_mod" ~ "Personality 6 CNI",
    cni_model == "pty7CNI_mod" ~ "Personality 7 CNI",
    TRUE ~ cni_model)) %>% 
  select(-level) 
...
swb.cni_results %>% 
   kable(booktabs = T,
        escape = F,
        format = "latex",
        col.names = c("CNI type", "Term", "Linear", "Quadratic"),
        align = c('l', 'c', 'c', 'c')) %>%
  collapse_rows(columns = 1, latex_hline = "linespace") %>%
  column_spec(2, width = "3em", latex_column_spec = "c") %>%
  column_spec(3, latex_column_spec = "c") %>%
  #column_spec(4, latex_column_spec = "c") %>%
  group_rows("Individual", 1, 12) %>%
  group_rows("Country", 13, 24)
```
...
```{r plot_swb-walk, echo=F, results='asis'}
#| cache: true
#| fig-height: 4
#| fig-width: 10
#| out-width: "100%"
...
# RQ4: Does personality CNI perform better than mindset CNI?
In order to assess which of the CNI's are preferable for predicting well-being outcomes, Anova Type III SS tests were computed to observe the effect of each predictor after accounting for all other predictors in the model.

```{r swb.cni-comparing-models}
#| cache: true
# Create a tibble with all combinations of two CNI types
cni_combinations <- combn(names(all_swb.cni_mods), 2) %>%
  t() %>%
  as_tibble(.name_repair = "minimal") %>%
  set_names(c("mod1", "mod2"))
...
cni_type <- gsub("_mod$", "", mod_name)
 ...
 join_data <- coef(cni_mod)$pID %>%
    ...
    full_join(data1, by = c("pID", "country")) %>% select(all_of(cni_type), pID, country, contains("sc_swb")) %>% 
   as_tibble()
 
 return(join_data)
}

fit_combined_model <- function(mod1, mod2) {
  mod1_name <- deparse(substitute(mod1))
  mod2_name <- deparse(substitute(mod2))
  
  mod1_data <- join_with_data(mod1)
  mod2_data <- join_with_data(mod2)
  
  mod1_cni <- gsub("_mod$", "", mod1_name)
  mod2_cni <- gsub("_mod$", "", mod2_name)
  
  combined_data <- mod1_data %>%
    full_join(mod2_data, by = c("pID", "country", "sc_swb.indv", "sc_swb.comm")) %>% 
  rename(!!mod1_cni := mod1,
           !!mod2_cni := mod2) %>%  
    na.omit()
  

  formula_combined <- as.formula(paste("sc_swb.indv ~", mod1_cni, "+", mod2_cni, "+ (1 | country)"))
  

   combined_model <- lmer(formula_combined, data = combined_data)

  fit <- Anova(combined_model, type = "III") %>%
    tidy() %>%
...
all_cni_mods <- lst(mindsetCNI_mod, ismCNI_mod, globeCNI_mod,fantcCNI_mod,
   pty6CNI_mod, pty7CNI_mod)

cni.swb_all_models <- map2_dfr(cni_combinations$mod1, cni_combinations$mod2, ~ {
  # Fetch model objects from the environment
  mod1_obj <- get(.x, envir = .GlobalEnv)
  mod2_obj <- get(.y, envir = .GlobalEnv)
  
  # Fit the combined model
  model_result <- fit_combined_model(mod1_obj, mod2_obj)
  
  # Add the model names to the result
 model_result <- model_result %>%
  ...
  mutate(across(where(is.character), ~ str_replace_all(., "_mod", "")))
...
cni.swb_all_models <- map2_dfr(cni_combinations$mod1, cni_combinations$mod2, ~ {
  model_result <- eval(call("fit_combined_model", as.name(.x), as.name(.y)))
...
```{r swb.cni-comparing-models-tbl, results='asis', echo=FALSE}
#| tbl-cap: "Comparing predictive capacity of CNI"
#| cache: true
cni.swb_all_models %>% 
  mutate(across(c(chisq, df), printnum)) %>% 
  ...
  kable(format = "latex",
        ...
        longtable = T,
        escape = F,
        col.names = c("Model", "Term", "Chisq", "df", "p" )) %>% 
  kable_styling() %>% 
   collapse_rows(columns = 1, latex_hline = "linespace")
```
...
all_combined_data <- map(names(all_cni_mods), function(mod_name) {
...
reduce(full_join, by = c("pID", "country", "sc_swb.indv", "sc_swb.comm")) %>%
...
# Auxiliary Question
This question brought forth the possibility of an overlap between the Big Six affecting the relationship between CNI and well-being. The correlation effects were small indicating that there is low overlap between Personality-CNI variables and predictors such as well-being affect the observed results.

```{r results='asis', echo=FALSE, eval=TRUE}
#| label: tbl-swbcor
#| tbl-cap: "Correlation between Subjective Well-being and Big Six and Disintegration"
#| cache: true
options(knitr.kable.NA = '')

data1 %>% 
  select(starts_with("scP_") & ends_with(".indv"), sc_swb.indv) %>% 
  cor() %>% 
  as_tibble(rownames = "variable") %>%
  select(variable, sc_swb.indv) %>%
  rename(correlation = sc_swb.indv) %>%
  mutate(
  ...
  filter(variable != "sc_swb.indv") %>%
  mutate(
    variable = case_when(
      str_detect(variable, "Extra") ~ "Extraversion",
      str_detect(variable, "Agree") ~ "Agreeableness",
      str_detect(variable, "Consc") ~ "Conscientiousness",
      str_detect(variable, "Hon") ~ "Honesty",
      str_detect(variable, "Res") ~ "Resiliency",
      str_detect(variable, "Dis") ~ "Disintegration",
      str_detect(variable, "Vir") ~ "Virtuosity/Openness",
      TRUE ~ variable)) %>% 
  kable(booktabs = T,
        escape = F,
        format = "latex",
        col.names = c("Variable", "Cor with SWB")) %>%
  kable_styling() %>%  
  add_footnote("Note: Correlation between scale scores for Big Six and Subjective Well-being at the individual level", notation = "none")
```
