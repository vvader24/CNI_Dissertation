...
all_mods_ctry.trends %>% 
  ...
  select(country, estimate, ci, p.value) %>% 
   kable(booktabs = T, 
        ...
        caption = "Country estimates across CNI types") %>% 
...
specify_var_names <- \(data){
  data %>% 
    ...
    modtr = case_when(
      ...
      str_detect(modtr, "globe_in") ~ "Ingroup_Collectivism",
...
#| tbl-cap: "Descriptive Statistics for Effect size Estimates"
library(e1071)
...
#| fig-cap: "Distribution of Effects"
#| fig-width: 7
#| fig-height: 4
...
df1 <- data %>% 
  #cni.ContMod_tables$pty7CNI.mindset %>% 
...
cni.ContMod_tbl_details <-  cni.ContMod_tblNames %>% 
  mutate(caption_part = rep(pull(cni_plot_details, caption), each = 2),
         caption = paste0("Linear Associations between ",caption_part, " and ", variable_type, " traits"),
...
plot_high.low_mindset_effects <- function(data) {
    ...
    TRUE ~ level  # This keeps other values unchanged
      ...
      color = case_when(
        ...
        modtr %in% c("Traditional family structure", "Uncertainty avoidance", "Achievement orientation",
                     "Assertiveness", "Humane", "Masculinity", "Ingroup_Collectivism", "Power distance", "Future orientation") ~ "Globe",
  ...
  # Define the plot
  combined_plot <- ggplot(all_data, aes(x = reorder(modtr, estimate), y = estimate, color = color, linetype = term_type, shape = level)) +
...
cni.pty_effects_plot <- function(data) {
  data %>% 
    #only linear effects
    filter(term == "z_ctry_response:modtr_score") %>% 
    specify_var_names() %>% 
...
cni.mindset_effects_plot <- function(data) {
  data %>% 
    #only linear effects
    filter(term == "z_ctry_response:modtr_score") %>%
    specify_var_names() %>%
    ...
    mutate(
      ...
      color = case_when(
        ...
        modtr == "Ingroup_Collectivism" ~ "Globe",
