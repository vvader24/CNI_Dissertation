...
pty6CNI_items <- get_item_names("pty", all_vars_d)
pty7CNI_items <- get_item_names("pty", all_vars_d)
...
```{r}
patch_CNIplots(cogCNI_plot, "cognitive CNI")
```

```{r}
patch_CNIplots(ismCNI_plot, "Isms-Cognitive CNI")
```

```{r}
patch_CNIplots(globeCNI_plot, "Globe-Cognitive CNI")
```

```{r}
patch_CNIplots(fantcCNI_plot, "Fanaticism-Cognitive CNI")
```

```{r}
patch_CNIplots(pty6CNI_plot, "Personaliy6 CNI")
```

```{r}
patch_CNIplots(pty7CNI_plot, "Personaliy7 CNI")
```
...
```{r swb.cni-lmer, eval=FALSE, echo=FALSE}
cni_mod=ismCNI_mod
...
#-------------------------# Aug 14 11:50am Test for heteroskedasticity:
