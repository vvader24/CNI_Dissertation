...
data1 %>% 
  ...
  arrange(desc(N)) %>% 
  kable(format = "latex", 
        col.names = c("Country", "Mean Age", "% Male", "\\textit{n}"), 
        booktabs = TRUE)
```
...
options(knitr.kable.NA = '')
 ...
 MI_table %>% 
  kable(format = "html", 
        booktabs = TRUE, 
        escape = FALSE, 
        col.names = c("Invariance type", "Chi square", colnames(MI_table)[3:6], "\u394 CFI", " \u394 RMSEA", "\u394 SRMR")) %>%
...
#| tbl-cap: "Cognitive and Perosnality Cultural Normativity Index (CNI) Models"
CNImods_table1 
```
...
# RQ 2: Associations between CNI and personality and cognitive variables
...
#| fig-cap: 
#|   !expr cni.ContMod_tbl_details %>%
#|   filter(variable_type == "cognitive") %>% 
#|     pull(caption)
