...
Given the large _n_, especially for mindsetCNI profiles and the total number of moderators, it will be necessary to use parallel processing for this analysis. It significantly reduces computation time for multiple independent tasks.
...
quick_strong.weak_effects_table(all_cni.contMod_effects$mfCNI.mindset_effects, pull(cni.contMod_tbl_details, "caption2")[11])
...
### Overall trends
Similar to study 1, three categories of tables are presented here:
...
```{r mindsetCNI.catMod-trends-plots, results='asis'}
# Moderator names
...
Here one function computes the model, assesses the model fit using the Type III sum of squares (Type III because the focus is on to what extent does the moderator interaction explain variability in the model, above and beyond other main and random effects). The significant contrasts for each model are retained. 
