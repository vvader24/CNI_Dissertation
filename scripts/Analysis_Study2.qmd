...
specify_var_names <- \(data){
  data %>% 
    ...
    modtr = case_when(
      ...
      str_detect(modtr, "globe_pf") ~ "Achievement Orientation", #Performance/
...
cni.ContMod_names <- tibble(objects = str_remove(cni.contMod_object_names, "\\.Rdata$")) %>% 
  ...
  mutate(title = paste0(part1, " CNI ~ ", part2),
         caption = paste0("Linear associations between ", part1, "CNI and ", part2, " traits. Bars: standardized interaction effect (CNI by trait). Errorbars: 95% CI"))%>% 
  select(objects, title, caption) 
...
processed_data <- data %>%
  ...
  guide = guide_legend(
    ...
    ncol = 1,
    direction = "vertical")) +
    labs(x = NULL, y = "Estimate", title = title, fill = "Trait", alpha = "Level")
  ...
  theme_bw() +
  theme(axis.text.y = element_markdown(size = 8.8), 
...
cogCNI being the most complex model, it required a different approach for running parallel processing. Here the models are computed first followed by assessing the contrasts between different groups. 
...
```{r cogCNI.catMod-effects-tables1, results='asis'}
# Moderator names
...
```{r cni.catMod-effects-tables2, results = 'asis'}
