...
pacman::p_load(tidyverse, dplyr, rio, expss, scales, glue,
               ...
               patchwork,gridExtra, ggplot2, grid, #plots
...
cni.cog_effects_plot <- function(data, title) {
  #data %>%
    all_cni.contMod_effects$cogCNI.cog_effects %>% 
    filter(term == "z_ctry_response:modtr_score") %>%
    specify_var_names() %>%
    with_groups(modtr, ~ mutate(.x, avg_abs_estimate = mean(abs(estimate)))) %>% 
    mutate(
      modtr = fct_reorder(modtr, avg_abs_estimate, .desc = FALSE),
      color = case_when(
      modtr == "Isms_TradRelig" ~ "Isms",
  modtr == "Isms_UnmiSelfInt" ~ "Isms",
  modtr == "CommRation" ~ "Isms",
  modtr == "SubjSpirit" ~ "Isms",
  modtr == "IneqAver" ~ "Isms",
  modtr == "globe_igc" ~ "Globe",
  modtr == "globe_ic" ~ "Globe",
  modtr == "globe_pf" ~ "Globe",
  modtr == "globe_un" ~ "Globe",
  modtr == "globe_ge" ~ "Globe",
  modtr == "globe_po" ~ "Globe",
  modtr == "globe_fu" ~ "Globe",
  modtr == "globe_ass" ~ "Globe",
  modtr == "globe_hu" ~ "Globe",
  modtr == "Social Cynicism" ~ "Social Axioms",
  modtr == "Reward for Application" ~ "Social Axioms",
  modtr == "Social Complexity" ~ "Social Axioms",
  modtr == "Fate Control" ~ "Social Axioms",
  modtr == "Religiosity" ~ "Social Axioms",
  modtr == "Harm" ~ "Moral Foundations",
  modtr == "Fairness" ~ "Moral Foundations",
  modtr == "Loyalty" ~ "Moral Foundations",
  modtr == "Authority" ~ "Moral Foundations",
  modtr == "Purity" ~ "Moral Foundations",
  modtr == "Horizontal Collectivism" ~ "Individualism-Collectivism",
  modtr == "Vertical Collectivism" ~ "Individualism-Collectivism",
  modtr == "Horizontal Individualism" ~ "Individualism-Collectivism",
  modtr == "Vertical Individualism" ~ "Individualism-Collectivism",
  modtr == "Cohesion" ~ "Family Values",
  modtr == "Hierarchy" ~ "Family Values",
  modtr == "Self Transcendence" ~ "Schwartz Values",
  modtr == "Conservation" ~ "Schwartz Values",
  modtr == "Self Enhancement" ~ "Schwartz Values",
  modtr == "Openness to Change" ~ "Schwartz Values",
  modtr == "Fanaticism" ~ "Fanaticism",
  modtr == "Material" ~ "Materialism",
  modtr == "Machiaveln" ~ "Machiavellianism",
  modtr == "Prone to Aggression" ~ "Prone to Aggression",
  modtr == "Ethnonational" ~ "Ethnonationalism",
  modtr == "Tightness" ~ "Tightness",
  modtr == "Amoralism" ~ "Amoralism",
  modtr == "DRI_OrgRelgAc" ~ "Duke Religiosity Index",
  modtr == "DRI_NonOrgRelgAc" ~ "Duke Religiosity Index",
  modtr == "DRI_IntrnscRelg" ~ "Duke Religiosity Index",
  modtr == "CivicMCNational" ~ "Civic Multicultural Nationalism",
    ...
    scale_fill_manual(values = c(
      ...
      "Schwartz Values" = "#A65628",
      "Fanaticism" = "#008f7a",
      "Materialism" = "#F781BF",
      "Machiavellianism" = "#999999",
      "Prone to Aggression" = "#E41A1C",
      "Ethnonationalism" = "#00FF00",
      "Tightness-Looseness" = "#8B4513",
      "Amoralism" = "#00FFFF",
      "Duke Religiosity Index" = "#FF1493",
      "Civic Multicultural Nationalism" = "#1E90FF"
    )) +
    scale_alpha_manual(values = c("Individual" = 1, "Community" = 0.4)) +
...
View(all_cni.contMod_effects$cogCNI.cog_effects)
```
...
table_catMods <- kable(df, format = "pipe", align = c("l", "l")) %>%
  kable_styling(full_width = FALSE) %>%
  row_spec(0, bold = TRUE) %>%
