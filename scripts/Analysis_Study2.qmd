...
cni.ContMod_effects_try <- function(cni_profiles_data, modtr_pattern) {
 ...
 results_quadratic <- cni.modtr_models %>% 
    select(modtr, tidy_mod.quadratic) %>% 
    unnest(cols = c(tidy_mod.quadratic)) %>%
    filter(term %in% c("z_ctry_response:I(modtr_score^2)", "z_ctry_response:modtr_score")) %>%
    select(modtr, term, estimate, conf.low, conf.high, p.value)%>% 
...
try = cni.ContMod_effects_try(pty6CNI_profiles$self.ctry_profiles, "^scP.*indv$")


names(all_cni.contMod_effects)
try %>% View()

cni.ContMod_effects_quad.b <- function(cni_profiles_data, modtr_pattern) {
  # Determine the level based on the modtr_pattern
  level <- if(grepl("indv", modtr_pattern)) "Individual" else if(grepl("comm", modtr_pattern)) "Country" else "Unknown"
  ...
  cni.modtr_models <- cni.modtr_data %>%
    ...
    mutate(
      ...
      #save results
      tidy_mod.quadratic = map(mod.quadratic, ~tryCatch(
        broom.mixed::tidy(., conf.int = TRUE),
        error = function(e) {
          message("Error in tidy: ", conditionMessage(e))
          NULL
        }
      ))
    )
...
try = cni.ContMod_effects_quad.b(ismCNI_profiles$self.ctry_profiles, "^scP.*indv$")
```


```{r}
## [1] "mindsetCNI.mindset_effects"
##  [2] "mindsetCNI.pty_effects"  
##  [3] "ismCNI.mindset_effects"    
##  [4] "ismCNI.pty_effects"        
##  [5] "globeCNI.mindset_effects"  
##  [6] "globeCNI.pty_effects"      
##  [7] "globeExCNI.mindset_effects"
##  [8] "globeExCNI.pty_effects"    
##  [9] "fantcCNI.mindset_effects"  
## [10] "fantcCNI.pty_effects"      
## [11] "mfCNI.mindset_effects"     
## [12] "mfCNI.pty_effects"         
## [13] "socAxCNI.mindset_effects"  
## [14] "socAxCNI.pty_effects"      
## [15] "pty6CNI.mindset_effects"   
## [16] "pty6CNI.pty_effects"     
```
...
mindsetCNI.mindset_effects_b<- future_map_dfr(var_names, function(var) {
  result <- cni.ContMod_effects_quad.b(mindsetCNI_profiles$self.ctry_profiles, var)
...
save(mindsetCNI.mindset_effects_b,
     file = here("objects/Study2/temp/mindsetCNI.mindset_effects_b.Rdata"))
```

```{r}
#Models I have run and saved:

#mindsetCNI.pty_effects_b
#mindsetCNI.mindset_effects_b

#ismCNI.pty_effects_b
#ismCNI.mindset_effects_b

#globeCNI.pty_effects_b
#globeCNI.mindset_effects_b

#globeExCNI.pty_effects_b
#globeExCNI.mindset_effects_b

#fantcCNI.pty_effects_b
#fantcCNI.mindset_effects_b

#mfCNI.pty_effects_b
#mfCNI.mindset_effects_b

#socAxCNI.pty_effects_b
#socAxCNI.mindset_effects_b

#pty6CNI.pty_effects_b
#pty6CNI.mindset_effects_b
